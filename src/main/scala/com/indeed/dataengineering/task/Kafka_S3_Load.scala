...
class Kafka_S3_Load extends Logging {
  ...
  def run(rawData: DataFrame, connector: CassandraConnector, partitions: Set[Int]): Unit = {
    ...
    def runJob(retryCnt: Int = 0): Unit = {
      ...
      try {
        ...
        case e: AmazonS3Exception =>
          if (retryCnt < 5) {
            log.error("Error: Encountered AmazonS3Exception")
            log.error(e.printStackTrace())
            log.info(s"Sleeping for ${conf.getOrElse("runInterval", "5")} minutes... and retrying...")
            Thread.sleep(conf.getOrElse("runInterval", "5").toInt * 60 * 1000)
            val end = DateTime.now
            if (Hours.hoursBetween(start, end).getHours > 1) runJob() else runJob(retryCnt + 1)
